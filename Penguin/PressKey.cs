...
namespace Penguin
{
    class PressKey
    {
        ...
        [Flags]
        internal enum PMKEYEVENTF : uint
        {
            KEY_DOWN = (0x0100),
        ...
        internal struct KEYBDINPUT
        {
            ...
            internal PMKEYEVENTF dwPMFlags;
            internal int time;
        ...
        ///     ie key_up, key_down
        /// wParam: This is the key that we are sending as an (uint) ... use scanCodeShort
        ...
        public static void Post_Message(ScanCodeShort key, PMKEYEVENTF flag, VirtualKeyShort virtualKey)
        {
            WinHandleMethods winHandle = new WinHandleMethods(CharacterObject.gameWindowName);
            int processNumber = 0;
            IntPtr gameHandle= winHandle.getGameWindowHandle();
            MessageBox.Show(key.ToString());
            MessageBox.Show(flag.ToString());
            MessageBox.Show(virtualKey.ToString());
            IntPtr lParam = (IntPtr)getLParam(key, (uint)0, (uint)PMKEYEVENTF.KEY_DOWN, (uint)1, (uint)1);
            int worked = (int)PostMessage(gameHandle, (uint)virtualKey, (IntPtr)ScanCodeShort.KEY_D, lParam);  
        }

        public static uint getLParam(ScanCodeShort scanCodeShort, uint extended, uint contextCode, uint previousState, uint transitionState)
        {
            // 0-15
            uint repeatCount = 10;
            ...
            uint scanCode = (uint)scanCodeShort;
        ...
        public static void Send_Key(ScanCodeShort key, KEYEVENTF flag)
        {
            ...
            InputData[0].ki.time = 10;
            InputData[0].ki.dwExtraInfo = UIntPtr.Zero;

            int worked = (int)SendInput(1, InputData, Marshal.SizeOf(typeof(INPUT)));
            //if (worked == 0)
            //{
            //    MessageBox.Show("Failed");
            //}

            //INPUT[] hardwareData = new INPUT[1];
            //int numSix = 36;

            //hardwareData[0].hi.uMsg = numSix;
            //hardwareData[0].hi.wParamL = 0x04;
            //hardwareData[0].hi.wParamH = 0x02;

            //SendInput(1, hardwareData, Marshal.SizeOf(typeof(INPUT)));
        }
